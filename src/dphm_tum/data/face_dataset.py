...
from dphm_tum.data.utils import  uniform_ball
import dphm_tum.env_paths as env_paths
...
class ScannerJson(Dataset):
    def __init__(self,
        ...
        if mode == 'train':
            self.subjects = self.manager.get_train_subjects(neutral_type='closed', exclude_missing_neutral=False)
        else:
            self.subjects = self.manager.get_eval_subjects(neutral_type='closed', exclude_missing_neutral=False)

    
        with open(subject_json_path, 'r') as f:
            subjects = json.load(f)  
        with open(expression_json_path, 'r') as f: 
            expressions = json.load(f)
        # parse expression json file
        expressions_for_all_subjects = []
        expressions_for_each_subject =[]
        for i in range(len(expressions)):
            if i == len(expressions) - 1:
                expressions_for_each_subject.append(expressions[i])
                expressions_for_all_subjects.append(expressions_for_each_subject)
            elif expressions[i] < expressions[i+1]:
                expressions_for_each_subject.append(expressions[i]) 
            else:
                expressions_for_each_subject.append(expressions[i]) 
                expressions_for_all_subjects.append(expressions_for_each_subject)
        ...
        print(f'Dataset has  {len(expressions)} Expressions!')
        #print(self.subjects)

        self.subject_steps = [] # stores subject id for each data point
        self.expression_steps = [] # stores expression if for each data point #self.steps = [] 
        self.subject_index = [] # defines order of subjects used in training, relevant for auto-decoder
        self.expression_index = [] # defines order of subjects used in training, relevant for auto-decoder

        all_files = []
        count = 0
        for i, s in enumerate(self.subjects):
            expressions = expressions_for_all_subjects[i]

            self.subject_steps += len(expressions) * [s, ]
            self.subject_index += len(expressions) * [i, ]
            self.expression_steps += expressions  
            self.expression_index += [ei + count for ei in range(len(expressions))]
            count += len(expressions)
            all_files.append(expressions)
    
        print('number of expressions :', len(self.expression_steps))
        self.batch_size = batch_size
    ...
    def __getitem__(self, idx):
        ...
        if gt_anchors is None:
            ...
            if self.lm_inds is not None:
                ...
                gt_anchors = np.zeros([39, 3])
        
        return_dict = {
            'idx': np.array([idx]),
            'iden': np.array([self.subjects.index(iden)]), # same as subj_ind ???
            'expr': np.array([expr]),
