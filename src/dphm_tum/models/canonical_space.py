...
class NPHM(MessagePassing):
    ...
    def forward(self, in_dict, debug_regio=False, squeeze=False, skip_color=False) -> (Tensor, Tensor):
    ...
    def message(self,
                ...
                skip_color=False) -> (Tensor, Tensor):
    ...
    def get_reg_loss(self, latent_code, stds = None):
        stds = None
        loss_dict = {}
        if stds is not None and stds['geo'] is not None:
            loss_dict['reg_loc_geo'] = ((latent_code['geo'][..., self.lat_dim_glob:]**2)/stds['geo'][self.lat_dim_glob:]).sum(dim=-1).mean() * stds['geo'][self.lat_dim_glob:].mean()
        else:
        ...
        if stds is not None and stds['app'] is not None:
            loss_dict['reg_loc_app'] = ((latent_code['app'][..., self.lat_dim_glob:] ** 2) / stds['app'][
                                                                                             self.lat_dim_glob:]).sum(dim=-1).mean() * \
                                       stds['app'][self.lat_dim_glob:].mean()
        ...
        loss_dict['reg_unobserved_geo'] = 0
        for idx in unobserved_idx:
            loss_dict['reg_unobserved_geo'] += torch.norm(latent_code['geo'][..., self.lat_dim_glob+idx*self.lat_dim_loc_geo:self.lat_dim_glob+(idx+1)*self.lat_dim_loc_geo], dim=-1).square().mean()
        ...
        #return ((5*loss_dict['reg_loc_geo'] + loss_dict['reg_global_geo']) + (5*loss_dict['reg_loc_app'] + loss_dict['reg_global_app']))#/2
        return loss_dict
...
def get_id_model(cfg,
    ...
    id_model = NPHM(
        ...
        lat_dim_loc=cfg_id['lat_dim_loc_geo'] + cfg_id['lat_dim_loc_app'] if include_color_branch else cfg_id['lat_dim_loc_geo'],
        ...
        rank=rank,
        is_monolith=False,
        blend_std=cfg_id['blend_std'] if 'blend_std' in cfg_id else 4, # no blend_std in old config
        )
