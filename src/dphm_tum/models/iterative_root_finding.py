import torch
from dphm_tum.models.diff_operators import jac, gradient
...
def nabla(decoder_shape, xc, cond, anchors):
    ...
    out = decoder_shape({'queries_can': xc, 'cond': cond, 'anchors': anchors})
    return out['sdf'], gradient(out['sdf'], xc)
...
def search(obs, cond, decoder_expr, anchors, multi_corresp=True):
    ...
    if multi_corresp:
        ...
        offsets = torch.randn(xc_init.shape, device=xc_init.device) * 0.002
        offsets[:, :, 0, :] = 0
        xc_init += offsets
        xc_init = xc_init.reshape(n_batch, n_point*num_inits, 3)

        #obs = obs.repeat_interleave(num_inits, dim=1)
        obs = obs.repeat(1, num_inits, 1)
        for k in cond.keys():
            cond[k] = cond[k][:, 0, :].unsqueeze(1).repeat(1, xc_init.shape[1], 1)
    ...
    in_dict = {'queries': xc_init, 'cond': cond, 'anchors': anchors}
    # compute init jacobians
    J_inv_init = jac(decoder_expr, in_dict).inverse()
    ...
    def _func(xc_opt, mask=None):
        if multi_corresp:
            ...
            out = decoder_expr({'queries': xc_opt.reshape(1, xc_opt.shape[0], 3), 'cond': cond, 'anchors': anchors})
            xd_opt = out['offsets'] + xc_opt.reshape(1, xc_opt.shape[0], 3)
        else:
            out = decoder_expr({'queries': xc_opt.reshape(1, xc_opt.shape[0], 3), 'cond': cond, 'anchors': anchors})
            xd_opt = out['offsets'] + xc_opt.reshape(1, xc_opt.shape[0], 3)
        if obs.shape[0] != 1:
    ...
    if multi_corresp:
        ...
        result["diff"] = result["diff"].reshape(n_batch, n_point, num_inits)
    else:
