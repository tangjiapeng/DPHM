...
def create_grid_points_from_bounds(minimun, maximum, res, scale=None):
    '''
    Creates a 3-dim grid of 3D points that uniformly covers the cuboid,
        that is spanned by the values defined in "minimum" and "maximum".
    "Res" specifies the resolution
    :param minimun:
    :param maximum:
    :param res:
    :param scale:
...
def mesh_from_logits(logits, mini, maxi, resolution, thresholds=None):
    '''
    Given the 1D-tensor of SDF predctions, reshape it back to 3D and perform marching cubes,
        in order to reconstruct the iso-surface at the specified "threshold".
    :param logits:
    :param mini:
    :param maxi:
    :param resolution:
